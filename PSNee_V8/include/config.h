...
#define GLOBALE_INTERRUPT_ON      sei()                                   //Set global interrupt enable SREG |=(1<<I)
#define GLOBALE_INTERRUPT_OFF     cli()                                   //Set global interrupt disable SREG &= ~(1<<I)

/* Handling the main pins
// Main pins input
//Create a mask (1<<0) with the first bit at 1 b00000001 uses the ~ operator to perform a bit inversion b11111110 , &= updates the DDRB register with the AND operator and the mask, DDRB bxxxxxxxx OR mask b11111110 = bxxxxxxx0
#define DATA_INPUT      DDRB  &= ~(1<<DDB0)                             //INPUT     D8
#define WFCK_INPUT      DDRB  &= ~(1<<DDB1)                             //INPUT     D9
#define SQCK_INPUT      DDRD  &= ~(1<<DDD6)                             //INPUT     D6  PSX subchannel clock
#define SUBQ_INPUT      DDRD  &= ~(1<<DDD7)                             //INPUT     D7   PSX subchannel bits
//Main pin output
...
#define DATA_OUTPUT     DDRB  |=  (1<<DDB0)                             //OUTPUT    D8 
#define WFCK_OUTPUT     DDRB  |=  (1<<DDB1)                             //OUTPUT    D9
...
#define DATA_SET        PORTB |=  (1<<PB0)                              //Pull_high D8
//Define pull-ups and set down at the main pin
//Create a mask (1<<0) with the first bit at 1 b00000001 uses the ~ operator to perform a bit inversion b11111110 , &= updates the DDRB register with the AND operator and the mask, DDRB bxxxxxxxx OR mask b11111110 = bxxxxxxx0
#define DATA_CLEAR      PORTB &= ~(1<<PB0)                              //Pull_down D8
#define WFCK_CLEAR      PORTB &= ~(1<<PB1)                              //Pull_down D9
//Read the main pins
...
#define SQCK_READ       (PIND & (1<<PIND6))                             //Read      D6
#define SUBQ_READ       (PIND & (1<<PIND7))                             //Read      D7
#define WFCK_READ       (PINB & (1<<PINB1))                             //Read      D9
...
void Init()
{
	...
	LED_USE;
	sei();                                                              //Set global interrupt enable SREG |=(1<<I)
}
